...
def parse_args():
    ...
    parser.add_argument(
        "--analyse", "--analyze", action="store_true", dest="analyse",
        help="skip the MD and analyse an existing csv",
    )
    parser.add_argument(
        "--atm-correction", action="store_true",
    ...
    parser.add_argument("--n-frames", type=int, default=25,
                        help="frames used by --atm-correction")
    ...
    parser.add_argument("--barostat", choices=BAROSTATS, default="auto",
                        help="auto picks mc for tasks without a stress tensor "
                             "(e.g. omol), mtk otherwise")
    parser.add_argument("--mc-interval", type=int, default=25,
                        help="steps between Monte Carlo volume moves (--barostat mc)")
    parser.add_argument("--mc-volume-scale", type=float, default=1.0,
                        help="initial MC volume move size in A^3 (--barostat mc)")
    parser.add_argument("--n-side", type=int, default=None,
                        help="box size tag matching stage 1's --n-side, if not default")
    parser.add_argument("--n-steps", type=int, default=NPT_STEPS,
    ...
    parser.add_argument("--outdir", default=".",
                        help="where the stage 1 inputs live and the outputs go")
