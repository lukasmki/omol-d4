...
def make_barostat(atoms, barostat="mtk", task=TASK, temperature_K=TEMPERATURE_K,
                  pressure_au=PRESSURE, timestep=TIMESTEP, friction=FRICTION,
                  mc_interval=25, mc_volume_scale=1.0):
    ...
    print(f"  Barostat: Monte Carlo (energy-only, interval={mc_interval}, "
...
class VolumeRecorder:
    ...
    HEADER = (f"{'Time[ps]':<10} {'Etot[eV]':>12} {'Epot[eV]':>12} "
    ...
    def __call__(self):
        ...
        line = (f"{time_ps:<10.4f} {epot + ekin:12.4f} {epot:12.4f} "
...
def load_equilibrated(three_body=False, task=TASK, model=MODEL, n_side=None,
                      outdir="."):
    ...
    print(f"Loaded {n_molecules(atoms)} H2O ({len(atoms)} atoms), "
          f"L = {atoms.cell.lengths()[0]:.2f} A")
    return atoms


def run_npt(steps=NPT_STEPS, three_body=False, model=MODEL, task=TASK,
            functional=None, disp3_cutoff=DISP3_CUTOFF, device="cuda",
            seed=SEED, n_side=None, barostat="auto",
            temperature_K=TEMPERATURE_K, pressure_au=PRESSURE,
            timestep=TIMESTEP, friction=FRICTION,
            sample_interval=SAMPLE_INTERVAL, mc_interval=25,
            mc_volume_scale=1.0, outdir=".", atoms=None, calc=None):
    ...
    dyn = make_barostat(
        atoms, barostat=kind, task=task, temperature_K=temperature_K,
        pressure_au=pressure_au, timestep=timestep, friction=friction,
        mc_interval=mc_interval, mc_volume_scale=mc_volume_scale,
    )

    traj = Trajectory(str(paths.npt_traj), "w", atoms)
    dyn.attach(traj.write, interval=sample_interval)
    recorder = VolumeRecorder(dyn, atoms, paths.npt_log, paths.npt_csv,
                              mc_diagnostics=kind == "mc")
    dyn.attach(recorder, interval=sample_interval)

    print(f"Running {steps} steps ({steps * timestep / FS_PER_PS:.1f} ps) NPT "
